...
import { Graph, type GraphPlugin } from '../../graph'
import type { Cell, Node } from '../../model'
import './api'
import { GroupImpl } from './group'
import { content } from './style/raw'

// Angle utility functions (from x6-geometry)
const Angle = {
  toRad: (deg: number) => (deg * Math.PI) / 180,
  normalize: (angle: number) => (angle % 360) + (angle < 0 ? 360 : 0),
}
    ...
    if (children?.length > 0) {
      children.forEach((child: Cell) => {
        if (child.isEdge()) {
          ...
          vertices &&
            vertices.forEach((vertex: PointLike) => {
    ...
    if (children?.length > 0) {
      children.forEach((child) => {
        ...
        if (child.isEdge()) {
          ...
          const newVertices: Array<Point> = []
          sVertices &&
            sVertices.forEach((vertex: PointLike) => {
              const vertice = new Point(vertex.x, vertex.y)
